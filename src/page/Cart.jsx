...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    try{
      const response = await fetch("http://laseappstore.test/api/allproduct");
      const data = await response.json();
      const matches = cart.map((cartItem) => {
  ...
  const increaseQuantity = (id) => {
    setProductMatches((prevMatches) => 
      prevMatches.map((item) => item.product_id === id ? {...item, q: item.q + 1} : item)
    );
  ...
  console.log("Product Match:", productMatches);
  return (
    <>
    <Header />
                  ...
                  <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
                    <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                      ...
                      <div className="flex items-center justify-between md:order-3 md:justify-end">
                        ...
                        <div className="text-end md:order-4 md:w-32">
                          <p className="text-base font-bold text-gray-900 dark:text-white">
                            {Number(item.q * item.selling_price).toLocaleString('en-NG', {style:'currency', currency:'NGN'})}
                          </p>
                        </div>
                      </div>
                ...
                {/* <div className="space-y-4">
                  <div className="space-y-2">
                    <dl className="flex items-center justify-between gap-4">
                     ...
                     <dl className="flex items-center justify-between gap-4">
                       <dt className="text-base font-normal text-gray-500 dark:text-gray-400">                         Savings
                       </dt>
